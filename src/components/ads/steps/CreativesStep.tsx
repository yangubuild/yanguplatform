  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col items-center gap-2 shrink-0">
          <div className="relative w-40 h-40">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-2xl font-bold text-foreground">{totalReach === "0" ? "0" : totalReach}</span>
              <span className="text-xs text-muted-foreground">/ 265M</span>
            </div>
          </div>
        </div>
          ...
          {REACH_STATS.map((stat) => {
            ...
            return (
              <div key={stat.label} className="flex items-center justify-between">
                ...
                <span className={`text-sm ${highlighted ? "text-green-400 font-medium" : "text-muted-foreground"}`}>
                  {stat.reach}
                </span>
              </div>
            );
      ...
      <div
        ...
        <div>
          <span className="text-sm font-medium text-foreground">Bulk upload</span>
      ...
      <div className="rounded-xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center justify-between px-5 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <span className="text-sm font-medium text-foreground">Vertical video</span>
              <p className="text-xs text-muted-foreground">Short-form video ads optimized for mobile explore</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
      ...
      <div className="rounded-xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center justify-between px-5 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <span className="text-sm font-medium text-foreground">Display ads</span>
              <p className="text-xs text-muted-foreground">Clickable display banners across high-traffic web placements</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
      ...
      <div className="rounded-xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center justify-between px-5 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <span className="text-sm font-medium text-foreground">Search ads</span>
              <p className="text-xs text-muted-foreground">Appears in YANGU Explore search results based on keywords</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className={`text-xs px-3 py-1.5 rounded-lg ${data.searchAd ? "text-green-400 bg-green-400/10 border border-green-400/20" : "text-muted-foreground"}`} style={!data.searchAd ? { background: "rgba(255,255,255,0.06)" } : {}}>
              5M reach/mo
            </span>
            <Button
              variant="accent"
          ...
          <div className="px-5 pb-4 flex items-center gap-3">
            <div className="relative flex items-center gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/[0.06] flex-1">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-foreground font-medium truncate">{data.searchAd.surfaceTitle}</p>
                <p className="text-xs text-muted-foreground">{data.searchAd.category} · {data.searchAd.productType}</p>
              </div>
              <button onClick={removeSearchAd} className="text-muted-foreground hover:text-muted-foreground">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
        ...
        <ModalOverlay onClose={() => setModal({ type: "none" })}>
          <div className="rounded-2xl w-full max-w-xl" style={{ background: "#1a1a1a" }}>
            <div className="flex items-center justify-between p-5 border-b border-white/10">
              <h3 className="text-lg font-semibold text-foreground">Bulk upload creatives</h3>
              <button onClick={() => setModal({ type: "none" })} className="text-muted-foreground hover:text-foreground"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6">
              <div onClick={() => bulkFileRef.current?.click()} className="border-2 border-dashed border-white/15 rounded-xl p-12 flex flex-col items-center justify-center cursor-pointer hover:border-white/25 transition-colors">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center"><Film className="w-5 h-5 text-muted-foreground" /></div>
                  <div className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center"><Image className="w-5 h-5 text-muted-foreground" /></div>
                </div>
                <span className="text-sm text-muted-foreground">Drop files or click to select</span>
                <span className="text-xs text-muted-foreground mt-1">Videos (MP4, MOV) and images (PNG, JPG, GIF)</span>
              </div>
            </div>
            <div className="flex gap-3 p-5 border-t border-white/10">
              <Button variant="outline" onClick={() => setModal({ type: "none" })} className="flex-1 rounded-xl border-white/10 text-muted-foreground">Cancel</Button>
        ...
        <ModalOverlay onClose={() => setModal({ type: "none" })}>
          <div className="rounded-2xl w-full max-w-md" style={{ background: "#1a1a1a" }}>
            ...
            <div className="p-5 space-y-4">
              <button className="w-full flex items-center justify-between p-3 rounded-xl border border-white/10 text-sm text-muted-foreground">Upload from my computer<span className="text-muted-foreground">∧</span></button>
              <div onClick={() => { if (fileRef.current) { fileRef.current.accept = "video/*"; fileRef.current.click(); } }} className="border-2 border-dashed border-white/15 rounded-xl p-10 flex flex-col items-center justify-center cursor-pointer hover:border-white/25 transition-colors">
                <div className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center mb-4"><Film className="w-5 h-5 text-muted-foreground" /></div>
                <span className="text-sm text-muted-foreground">Choose a file or drag and drop here</span>
                <span className="text-xs text-muted-foreground mt-1">.MP4 and .MOV formats</span>
              </div>
              <button className="w-full flex items-center justify-between p-3 rounded-xl border border-white/10 text-sm text-muted-foreground">Choose from my creatives library instead<span className="text-muted-foreground">∨</span></button>
            </div>
            <div className="flex gap-3 p-5 border-t border-white/10">
              <Button variant="outline" onClick={() => setModal({ type: "none" })} className="flex-1 rounded-xl border-white/10 text-muted-foreground">Cancel</Button>
          ...
          <div className="rounded-2xl w-full max-w-md" style={{ background: "#1a1a1a" }}>
            ...
            <div className="p-5 space-y-4">
              ...
              <div onClick={() => { if (fileRef.current) { fileRef.current.accept = "image/*"; fileRef.current.click(); } }} className="border-2 border-dashed border-white/15 rounded-xl p-10 flex flex-col items-center justify-center cursor-pointer hover:border-white/25 transition-colors">
                <div className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center mb-4"><Image className="w-5 h-5 text-muted-foreground" /></div>
                <span className="text-sm text-muted-foreground">Choose a file or drag and drop here</span>
                <span className="text-xs text-muted-foreground mt-1">.PNG and .JPG formats up to 25MB</span>
              </div>
              <button className="w-full flex items-center justify-between p-3 rounded-xl border border-white/10 text-sm text-muted-foreground">Choose from my creatives library instead<span className="text-muted-foreground">∨</span></button>
            </div>
            <div className="flex gap-3 p-5 border-t border-white/10">
      ...
      {modal.type === "crop" && (
        <ModalOverlay onClose={() => setModal({ type: "none" })}>
          <div className="rounded-2xl p-6 w-full max-w-lg space-y-5" style={{ background: "#1a1a1a" }}>
            <h3 className="text-lg font-semibold text-foreground">Crop media</h3>
            ...
            <div className="flex justify-end gap-3">
              <Button variant="ghost" onClick={() => setModal({ type: "none" })} className="text-muted-foreground">Cancel</Button>
        ...
        <ModalOverlay onClose={() => setModal({ type: "none" })}>
          <div className="rounded-2xl w-full max-w-4xl flex flex-col" style={{ background: "#1a1a1a", maxHeight: "90vh" }}>
            ...
            <div className="text-center py-5 border-b border-white/10 shrink-0">
              <h3 className="text-lg font-semibold text-foreground">Confirm Creative</h3>
            </div>
            ...
            <div className="flex flex-1 min-h-0">
              ...
              <div className="w-1/2 p-6 border-r border-white/10">
                <h4 className="text-sm font-semibold text-foreground mb-4">Media</h4>
              ...
              <div className="w-1/2 p-6 flex flex-col min-h-0">
                <h4 className="text-sm font-semibold text-foreground mb-4">Caption</h4>
                ...
                {writeOwn && (
                  <textarea
                    ...
                    rows={5}
                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-foreground placeholder:text-muted-foreground outline-none resize-none mb-4 shrink-0"
                ...
                {aiLoading && (
                  <div className="flex-1 flex flex-col items-center justify-center gap-4">
                    ...
                    <span className="text-sm text-muted-foreground">Generating AI captions...</span>
                  </div>
                )}
      ...
      {modal.type === "search-ads" && (
        <ModalOverlay onClose={() => setModal({ type: "none" })}>
          <div className="rounded-2xl w-full max-w-lg" style={{ background: "#1a1a1a" }}>
            ...
            <div className="p-5">
              <p className="text-sm text-muted-foreground mb-4">Select a published business to promote on Explore</p>
              {surfacesLoading ? (
                ...
                <div className="text-center py-10">
                  ...
                  <p className="text-sm text-muted-foreground">No published businesses found</p>
                  <p className="text-xs text-muted-foreground mt-1">Publish a business first to use Search ads</p>
                </div>
              ) : (
          ...
          <div className="rounded-2xl w-full max-w-lg" style={{ background: "#1a1a1a" }}>
            ...
            <div className="p-5 space-y-4">
              <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                ...
                <div>
                  <p className="text-sm text-foreground font-medium">{modal.surface.title}</p>
                  <p className="text-xs text-muted-foreground">/{modal.surface.slug}</p>
                </div>
                <Check className="w-5 h-5 text-green-400 ml-auto" />
              </div>

              <div>
                <label className="text-xs font-medium text-muted-foreground block mb-1.5">Product type</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground block mb-1.5">Category of business</label>
                <input
                  type="text"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground outline-none resize-none"
                  placeholder="Briefly describe what your business offers..."
                />
              </div>
            </div>
            <div className="flex gap-3 p-5 border-t border-white/10">
              <Button variant="outline" onClick={() => setModal({ type: "search-ads" })} className="flex-1 rounded-xl border-white/10 text-muted-foreground">Back</Button>
              <Button
                variant="accent"
