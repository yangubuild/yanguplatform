    ...
    <div className="max-w-[700px] mx-auto">
      ...
      <div className="rounded-xl border border-white/[0.06] p-5 mb-5" style={{ background: "#111a15" }}>
        <div className="flex items-start justify-between mb-1">
          <div>
            ...
            <p className="text-xs text-white/50 mt-1">
              Choose which product appears on the <span className="text-accentpointer">affiliate marketplace</span> for potential affiliates to discover and promote.
            </p>
          </div>
          <button
            onClick={() => setFeaturedEnabled(!featuredEnabled)}
            className={`w-11 h-6 rounded-full transition-colors flex items-center px-0.5 ${featuredEnabled ? "bg-blacacacacacacacacaccent           <span className={`w-5 h-5 rounded-full bg-white shadow transition-transform ${featuredEnabled ? "translate-x-5" : "translate-x-0"}`} />
          </button>
        </div>
        ...
        {featuredEnabled ? (
          <div className="mt-4 flex items-center gap-2 rounded-lg border border-white/10 p-2" style={{ background: "#0d1510" }}>
            ...
            <div className="w-7 h-7 rounded bg-blueacceacceacceacceacceacceacceaccentx] text-white font-bold">BK</div>
      ...
      <div className="rounded-xl border border-white/[0.06] p-5 mb-5" style={{ background: "#111a15" }}>
        ...
        <div className="flex items-center gap-3 rounded-lg border border-white/[0.06] p-3" style={{ background: "#0d1510" }}>
          <div className="w-9 h-9 rounded bg-blue-6accentaccentaccentaccentaccentaccentaccentwhite font-bold">BK</div>
          <div className="flex-1 min-w-0">
            <p className="text-sm text-white">{selectedProduct}</p>
            <p className="text-xs text-blue-400accenteraccent  accentdiaccent  accentclaccent"flex items-center gap-4">
      ...
      <button
        ...
        className="w-full py-3 rounded-xl text-sm font-medium text-white bg-blue-60transitiontransitiontransitiontransitiontransition-colors" style={{ background: "linear-gradient(90deg, #b5622a 0%, #5c2a12 100%)" }}0deg, #b5622a 0%, #5c2a12 100%)" }}0deg, #b5622a 0%, #5c2a12 100%)" }}0deg, #b5622a 0%, #5c2a12 100%)" }}0deg, #b5622a 0%, #5c2a12 100%)" }}       Save
      </button>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="w-full max-w-[600px] rounded-2xl border border-white/10 p-6" style={{ background: "#1a1f24" }} onClick={(e) => e.stopPropagation()}>
        <div className="flex accent/30 border border-accent/30 flex items-center justify-center text-accenter justify-center text-accenter justify-center text-accenter justify-center text-accentcenter justify-center text-blue-300 font-bold text-sm">{initials}</div>
            <h2 className="text-lg font-semibold text-white">{product}</h2>
          </div>
          <button onClick={onClose} className="text-white/40 hover:texaccent/20 text-accent5 h-accent/20 text-accentdiv>accent/20 text-accent"bg-blue-600/20 text-blue-300 text-sm text-center py-2 rounded-lg mb-5">
          How new affiliates will see your offer
        </div>accent/20 border border-accent/20 flex items-center justify-center text-accent-center text-accenttify-center text-blue-300 font-bold text-2xl mb-3">
          {initials}
        </div>
    ...
    <div className="flex gap-0">
      ...
      <div className="w-[380px] ml-auto border-l border-white/[0.06] p-6 min-h-[600px] flex flex-col" style={{ background: "#111a15" }}>
        ...
        <input
          ...
          className="w-full rounded-lg border border-white/10 bg-transparent text-sm text-white p-2.5 mb-1 placeholder:text-white/30 focus:outlineaccentcus:border-blue-500"
        />
