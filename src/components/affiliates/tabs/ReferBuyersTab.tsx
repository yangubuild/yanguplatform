    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h3 className="text-base font-semibold text-foreground">Hot offers</h3>
            ...
            <div className="flex gap-3 p-4 pb-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-1">
                  <p className="text-sm font-medium text-foreground leading-tight">{offer.name}</p>
                  <button
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {offer.category} • <span className="text-muted-foreground bg-white/[0.06] px-1.5 py-0.5 rounded text-[10px]">{offer.type}</span>
                </p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-y-2 text-[11px] px-4 pb-4">
              <div><span className="text-muted-foreground">Product price</span><br /><span className="text-muted-foreground font-medium">{offer.price}</span></div>
              <div><span className="text-muted-foreground">Commission rate</span><br /><span className="text-muted-foreground font-medium">{offer.rate}</span></div>
              <div><span className="text-muted-foreground">Affiliate sales</span><br /><span className="text-muted-foreground font-medium">{offer.sales}</span></div>
              <div><span className="text-muted-foreground">Affiliate earnings</span><br /><span className="text-muted-foreground font-medium">{offer.earnings}</span></div>
              <div><span className="text-muted-foreground">Conversion rate</span><br /><span className="text-muted-foreground font-medium">{offer.conversion}</span></div>
              <div><span className="text-muted-foreground">Earnings per click</span><br /><span className="text-muted-foreground font-medium">{offer.epc}</span></div>
            </div>
          </div>
        ))}
      </div>
      ...
      {isAuthenticated && (
        <>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-foreground">Your affiliate programs</h3>
          ...
          {allJoinedOffers.length === 0 ? (
            ...
            <div className="rounded-xl border border-white/[0.04] overflow-hidden" style={{ background: "#111a15" }}>
              <div className="flex items-center border-b border-white/[0.04] px-4 py-3">
                <div className="flex-[2] text-xs text-muted-foreground font-medium">Company</div>
                <div className="flex-1 text-xs text-muted-foreground font-medium text-center">Clicks</div>
                <div className="flex-1 text-xs text-muted-foreground font-medium text-center">Conversions</div>
                <div className="flex-1 text-xs text-muted-foreground font-medium text-center">Earnings</div>
                <div className="flex-1 text-xs text-muted-foreground font-medium text-center">Assets</div>
              </div>
              {allJoinedOffers.map((prog) => (
                <div key={prog.id} className="flex items-center border-b border-white/[0.04] px-4 py-3 hover:bg-white/[0.02] transition-colors">
                  <div className="flex-[2] flex items-center gap-3">
                    ...
                    <span className="text-sm text-foreground font-medium">{prog.name}</span>
                  </div>
                  <div className="flex-1 text-sm text-muted-foreground text-center">0</div>
                  <div className="flex-1 text-sm text-muted-foreground text-center">0</div>
                  <div className="flex-1 text-sm text-muted-foreground text-center">$0.00</div>
          ...
          <h3 className="text-base font-semibold text-foreground mt-8 mb-4">Your affiliate links</h3>
          {allJoinedOffers.length === 0 ? (
            <AffEmptyTable
                      ...
                      <tr key={i} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => { navigator.clipboard.writeText(link.link); toast.success("Affiliate link copied!"); }} className="text-muted-foreground hover:text-foreground flex-shrink-0">
                              <Copy className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-sm text-muted-foreground text-right">$0.00</td>
                        <td className="px-4 py-3 text-sm text-muted-foreground text-center">0</td>
                        <td className="px-4 py-3 text-sm text-muted-foreground text-center">0</td>
          ...
          <h3 className="text-base font-semibold text-foreground mt-8 mb-4">Pending applications</h3>
          <AffEmptyTable
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto" style={{ background: "#08120D" }}>
      <div className="w-full px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <h3 className="text-base font-semibold text-foreground">Your affiliate links — {programName}</h3>
            <button className="flex items-center gap-1.5 px-3 py-1 rounded-lg border border-dashed border-white/20 text-xs text-muted-foreground">
              <span className="text-muted-foreground">⊕</span> Link type
            </button>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-white/[0.06] w-56">
            <Search className="w-4 h-4 text-muted-foreground" />
