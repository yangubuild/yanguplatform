...
import { ChevronLeft, ChevronRight, Star, Users, Eye, Building2 } from "lucide-react";
...
interface Props {
  ...
  /** Live data from canonical search */
      ...
      <div className="h-[170px] overflow-hidden">
        {entity.cover_image_url ? (
          ...
          <div className="w-full h-full flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.03)' }}>
            <Building2 className="w-8 h-8" style={{ color: 'rgba(255,255,255,0.1)' }} />
          </div>
        )}
      </div>
      <div className="p-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              {badge === "green" && (
                <span className="w-4 h-4 rounded-full bg-green-600 flex items-center justify-center text-[8px] text-white shrink-0">✓</span>
              )}
            </div>
...
export function PremiumBusinessRow({ title, subtitle, businesses, entities }: Props) {
  ...
  // Prefer live entities, fall back to static businesses
