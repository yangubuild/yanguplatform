...
const PROCESS_STEPS = [
  {
    label: "WRITING",
    icon: (
      <svg width="40" height="40" viewBox="0 0 40 40" fill="none" className="text-[hsl(var(--destructive))]">
...
const STATS = [
  ...
  { value: "7 Days", label: "TURNAROUND", icon: <Clock className="w-5 h-5 text-[hsl(var(--destructive))]" />, highlight: true },
    ...
    <div className="space-y-5 mt-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl border border-border bg-card p-8 flex flex-col justify-center">
          <h2 className="text-2xl sm:text-3xl font-bold text-foreground leading-tight">
            Have your own{" "}
            <span className="text-[hsl(var(--destructive))]">Signature Product</span>{" "}
            created in 7 days.
          </h2>
          ...
          <div className="mt-6 flex items-center gap-4 flex-wrap">
            <a
              ...
              className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[hsl(var(--destructive))] text-white font-semibold text-sm hover:opacity-90 transition-opacity"
            >
      ...
      <div className="rounded-2xl border border-border bg-card p-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 sm:gap-12">
          ...
          <div className="flex-shrink-0">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-[hsl(var(--destructive))]/30 text-[hsl(var(--destructive))] text-xs font-semibold mb-2">
          ...
          <div className="flex items-center gap-8 sm:gap-12 flex-wrap flex-1 justify-center sm:justify-end">
            {PROCESS_STEPS.map((step) => (
              ...
              >
                <div className="w-16 h-16 rounded-full border border-border bg-card flex items-center justify-center transition-all duration-300 group-hover:border-[hsl(var(--destructive))]/40 group-hover:shadow-[0_0_20px_hsl(var(--destructive)/0.1)]">
                  {step.icon}
                </div>
                ...
                {/* Dot indicator */}
                <div className="w-1.5 h-1.5 rounded-full bg-border group-hover:bg-[hsl(var(--destructive))] transition-colors" />
              </div>
            ))}
          </div>
        </div>
        ...
        {STATS.map((stat) => (
          <div
            ...
            className={`rounded-2xl border p-5 flex items-center justify-between ${
              stat.highlight
                ? "border-[hsl(var(--destructive))]/40 bg-card"
            ...
            <div>
              <p
                className={`text-lg font-bold ${
                  stat.highlight ? "text-[hsl(var(--destructive))]" : "text-foreground"
                }`}
              >
                {stat.value}
              </p>
      ...
      <div className="rounded-2xl border border-border bg-card p-8">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <span className="text-xs font-semibold text-[hsl(var(--destructive))] tracking-wide">Limited Offer</span>
          ...
          <div className="text-center flex-shrink-0">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-3 inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-[hsl(var(--destructive))] text-white font-semibold text-sm hover:opacity-90 transition-opacity"
            >
              Get Started
              <ArrowRight className="w-4 h-4" />
            </a>
