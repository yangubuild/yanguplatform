...
import {
  ...
  Wand2, Clock, Eye, Mic, Settings2, Users, CheckCircle2, XCircle
} from "lucide-react";
  ...
  return (
    <>
          ...
          <div className="flex rounded-lg border border-border/40 overflow-hidden mb-3">
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-medium transition-colors ${
                voiceMode === "select" ? "bg-muted/50 text-foreground" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              Select Voice
            </button>
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-medium transition-colors border-l border-border/40 ${
                voiceMode === "upload" ? "bg-muted/50 text-foreground border border-primary/60 rounded-lg" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              Upload Audio
            </button>
      ...
      <VoiceLibraryModal
        ...
        onSelect={(v) => setSelectedVoice(v.name)}
      />
    </>
  );
}
    ...
    <div className="rounded-2xl border border-border/30 bg-card/40 p-6 space-y-8">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="e.g. living room, bedroom, kitchen, reading room, gym, office, clinic, pool, car, beach ..."
          className="w-full h-24 rounded-xl bg-muted/15 border border-border/30 px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground outline-none resize-none focus:border-primary/50"
        />
      ...
      <div className="rounded-2xl border border-border/30 bg-card/40 p-6">
        ...
        >
          ...
          <p className="text-xs text-muted-foreground mt-1 max-w-md text-center">
            You can upload videos up to 1000 MB in size. We support MP4, MOV, and WebM formats.
          </p>
          ...
          <div className="flex items-center gap-2 mt-4 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2 flex-1 px-3 py-2 rounded-lg bg-muted/20 border border-border/30">
              ...
              <input
                ...
                placeholder="Paste your Google Drive URL here"
  ...
  return (
    <div className={`relative ${className}`}>
      <button
        ...
        className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border text-sm text-foreground transition-colors ${
          open ? "border-primary/60 bg-muted/20" : "border-border/40 bg-muted/15 hover:border-border/60"
        }`}
      >
        <span>{value}</span>
...
function ChipRow({ chips, onSelect }: { chips: string[]; onSelect: (c: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-2 mt-2 overflow-hidden">
      <button
        onClick={() => onSelect(chips[Math.floor(Math.random() * chips.length)])}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-muted/30 border border-border/40 text-xs font-medium text-foreground shrink-0 hover:bg-muted/50 transition-colors"
      >
      ...
      <div ref={scrollRef} className="flex items-center gap-2 overflow-x-auto scrollbar-none">
        {chips.map((c) => (
          <button
            key={c}
            onClick={() => onSelect(c)}
            className="px-3 py-1.5 rounded-full border border-border/40 text-xs text-foreground whitespace-nowrap hover:bg-muted/30 transition-colors shrink-0"
          >
            {c}
          </button>
        ))}
      ...
      <button
        ...
        className="h-7 w-7 rounded-full border border-border/40 flex items-center justify-center shrink-0 hover:bg-muted/30 transition-colors"
      >
