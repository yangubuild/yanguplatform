 ...
 *
 * DISTRIBUTION: exactly 4 per key section:
 *   verified (Trusted Businesses): #1, #10, #11, #15
 *   products (Buy From):           #3, #5, #6, #7
 *   services (Find Services):      #2, #8, #12, #16
 *   community (Join Communities):  #4, #9, #13, #14
 *
 * Popular Businesses backfill draws from ALL 16 via getAllPlaceholderEntities().
 */

import type { SearchEntityResult } from "@/types/search";

// ── Cover image imports (all 16, each used exactly once) ──
...
export interface PlaceholderSurface {
  ...
  preferredSection: "verified" | "products" | "services" | "community";
}
...
// ── All 16 placeholder surfaces (4 per key section) ──

const ALL_PLACEHOLDERS: PlaceholderSurface[] = [
  // ═══════════════════════════════════════════
  // VERIFIED (Trusted Businesses) — 4 surfaces
  // ═══════════════════════════════════════════
  ...
  // 11. HavenSquare Properties
  {
    entity: {
      id: pid(11),
      entity_type: "business",
      entity_subtype: "general",
      title: "HavenSquare Properties",
  ...
  // ═══════════════════════════════════════
  // SERVICES (Find Services) — 4 surfaces
  // ═══════════════════════════════════════

  // 2. CoastRide Experiences
  {
    entity: {
      id: pid(2),
      entity_type: "service",
  ...
  // 16. Horizon Escape Travel
  {
    entity: {
      id: pid(16),
      entity_type: "service",
      entity_subtype: "general",
      title: "Horizon Escape Travel",
  ...
  // ═══════════════════════════════════════════
  // COMMUNITY (Join Communities) — 4 surfaces
  // ═══════════════════════════════════════════

  // 4. Heritage Circle Africa
  {
    entity: {
      id: pid(4),
  ...
  // 13. StreetPulse Footwear
  {
    entity: {
      id: pid(13),
      entity_type: "community",
      entity_subtype: "general",
      title: "StreetPulse Footwear",
  ...
  // 14. PureTone Skincare
  {
    entity: {
      id: pid(14),
      entity_type: "community",
      entity_subtype: "general",
      title: "PureTone Skincare",
...
export function backfillWithPlaceholders(
  ...
  // For "popular", draw from all 16 placeholders
  const sectionPlaceholders = section === "popular"
    ? getAllPlaceholderEntities()
    : getPlaceholdersForSection(section);

  const placeholders = sectionPlaceholders.filter((p) => !liveIds.has(p.id));
