...
export function InvoiceEmptyState({ onCreateInvoice }: InvoiceEmptyStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] px-4 bg-background">
      ...
      {/* Bottom action bar */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 px-4 sm:px-6 py-4 rounded-2xl max-w-2xl w-full bg-card border border-border mx-4 sm:mx-0">
        <p className="text-sm flex-1 text-muted-foreground hidden sm:block">
          Create your first invoice to request payment directly from a customer.
        </p>
        <button
          onClick={onCreateInvoice}
          className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium bg-primary text-primary-foreground shrink-0">
          <Plus className="w-4 h-4" />
          Create invoice
        </button>
        <button
          onClick={handleViewDocs}
          className="text-sm font-medium text-foreground shrink-0 px-3 py-2.5 hover:underline text-center">
          View docs
        </button>
      </div>
    </div>
