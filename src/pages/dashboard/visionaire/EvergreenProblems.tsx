...
export default function EvergreenProblems() {
  ...
  const { data: items, isLoading } = useVisionaireItems("evergreen");
  ...
  return (
    <VisionairePageContainer>
      <div className="space-y-6">
        <div>
          <h1 className="text-xl font-bold text-foreground">Evergreen Problems</h1>
          <p className="text-sm text-muted-foreground mt-1">Timeless niches and product ideas that sell year-round</p>
        </div>
        <div className="relative max-w-md">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input placeholder="Search niches..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
        </div>
