...
export default function VisionaireHome() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(
      (i) => i.title.toLowerCase().includes(q) || i.description?.toLowerCase().includes(q) || i.tags?.some((t: string) => t.toLowerCase().includes(q))
    );
  }, [items, search]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-foreground">Visionaire Library</h1>
        <p className="text-sm text-muted-foreground mt-1">Discover digital products, courses, templates and more</p>
      </div>
