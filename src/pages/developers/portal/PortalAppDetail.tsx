  ...
  if (!app) {
    return (
      <div className="text-center py-16">
        <p className="text-muted-foreground text-sm mb-4">App not found.</p>
...
function OverviewTab({ app }: { app: any }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {usage && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="rounded-xl p-4 bg-white/[0.02] border border-white/10">
            ...
            <p className="text-lg font-bold text-foreground">{usage.total_today}</p>
            <p className="text-[10px] text-muted-foreground">Today</p>
          </div>
          <div className="rounded-xl p-4 bg-white/[0.02] border border-white/10">
            <Activity className="w-4 h-4 mb-2 text-accent" />
            <p className="text-lg font-bold text-foreground">{usage.total_period}</p>
          ...
          <div className="rounded-xl p-4 bg-white/[0.02] border border-white/10">
            ...
            <p className="text-lg font-bold text-foreground">{usage.avg_latency_ms.toFixed(0)}ms</p>
      ...
      <div className="rounded-xl p-5 bg-white/[0.02] border border-white/10">
        <h3 className="text-foreground font-semibold text-sm mb-4">App Information</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <Label className="text-muted-foreground text-xs block mb-1">App Name</Label>
            {editing ? (
              <Input value={name} onChange={(e) => setName(e.target.value)} className="bg-white/5 border-white/10 text-foreground" maxLength={100} />
            ) : (
              <p className="text-foreground text-sm">{app.name}</p>
            )}
          </div>
          <div>
            <Label className="text-muted-foreground text-xs block mb-1">App Key</Label>
            <p className="text-muted-foreground text-sm font-mono">{app.slug}</p>
          </div>
          <div className="sm:col-span-2">
            <Label className="text-muted-foreground text-xs block mb-1">Description</Label>
            {editing ? (
              <Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="A short description" className="bg-white/5 border-white/10 text-foreground placeholder:text-muted-foreground" maxLength={500} />
            ) : (
              <p className="text-muted-foreground text-sm">{app.description || "No description"}</p>
            )}
          </div>
          <div>
            <Label className="text-muted-foreground text-xs block mb-1">Status</Label>
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">{new Date(app.created_at).toLocaleDateString()}</p>
          </div>
        </div>
        <div className="flex gap-2 mt-5">
          {editing ? (
            <>
              ...
              <Button variant="ghost" size="sm" onClick={() => { setEditing(false); setName(app.name); setDescription(app.description || ""); }} className="text-muted-foreground">Cancel</Button>
            </>
          ) : (
            <Button variant="secondary" size="sm" onClick={() => setEditing(true)} className="bg-white/5 border border-white/10 text-muted-foreground hover:bg-white/10">Edit</Button>
          )}
          <Button variant="ghost" size="sm" onClick={() => toggleStatus.mutate()} disabled={toggleStatus.isPending} className="text-muted-foreground ml-auto">
...
function OAuthTab({ appId }: { appId: string }) {
  ...
  return (
    <div className="rounded-xl p-5 bg-white/[0.02] border border-white/10">
      <h3 className="text-foreground font-semibold text-sm mb-1">OAuth Configuration</h3>
      <p className="text-muted-foreground text-xs mb-5">Manage redirect URIs for your OAuth flow.</p>

      <div className="space-y-3 mb-5">
        {(oauth?.redirect_uris ?? []).length === 0 ? (
          <p className="text-muted-foreground text-xs">No redirect URIs configured.</p>
        ) : (
          (oauth?.redirect_uris ?? []).map((uri) => (
            <div key={uri} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/[0.02] border border-white/5">
              ...
              <code className="flex-1 text-xs text-muted-foreground font-mono break-all">{uri}</code>
      ...
      <div className="flex gap-2">
        <Input
          ...
          className="bg-white/5 border-white/10 text-foreground placeholder:text-muted-foreground flex-1"
      ...
      {oauth?.scopes && oauth.scopes.length> 0 && (
        <div className="mt-6 pt-4 border-t border-white/10">
          ...
          <div className="flex flex-wrap gap-1">
            {oauth.scopes.map((s) => (
              <Badge key={s} variant="outline" className="text-[10px] border-white/20 text-muted-foreground">{s}</Badge>
            ))}
          </div>
        </div>
      )}
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-foreground font-semibold text-sm">Webhooks</h3>
      ...
      {showCreate && (
        <div className="rounded-xl p-5 space-y-4" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.10)" }}>
          <div>
            ...
            <Input value={newUrl} onChange={(e) => setNewUrl(e.target.value)} placeholder="https://example.com/webhooks" className="mt-1 bg-white/5 border-white/10 text-foreground" />
          </div>
          <div>
          ...
          <div className="flex gap-2">
            ...
            <Button variant="ghost" size="sm" onClick={() => setShowCreate(false)} className="text-muted-foreground">Cancel</Button>
          </div>
        </div>
      )}
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.10)" }}>
        {!webhooks?.length ? (
          <div className="text-center py-10">
            ...
            <p className="text-muted-foreground text-sm">No webhooks for this app.</p>
          </div>
        ) : (
            ...
            <tbody>
              {webhooks.map((wh: any) => (
                <tr key={wh.id} className="border-b border-white/5 last:border-0">
                  <td className="px-4 py-3 text-muted-foreground font-mono text-xs truncate max-w-[200px]">{wh.url}</td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      {(wh.events ?? []).map((ev: string) => (
                        <Badge key={ev} variant="outline" className="text-[10px] border-white/20 text-muted-foreground">{ev}</Badge>
                      ))}
                    </div>
                  </td>
...
function KeysTab({ appId }: { appId: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-foreground font-semibold text-sm">API Keys</h3>
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.10)" }}>
        {!keys?.length ? (
          <div className="text-center py-10">
            ...
            <p className="text-muted-foreground text-sm">No API keys for this app.</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead><tr className="border-b border-white/10">
              <th className="text-left px-4 py-3 text-muted-foreground font-medium text-xs">Prefix</th>
              <th className="text-left px-4 py-3 text-muted-foreground font-medium text-xs">Env</th>
              ...
              {keys.map((k: any) => {
                ...
                return (
                  <tr key={k.id} className="border-b border-white/5 last:border-0">
                    <td className="px-4 py-3"><span className="font-mono text-sm text-foreground">{k.prefix}</span></td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-muted-foreground text-xs">{new Date(k.created_at).toLocaleDateString()}</td>
                    <td className="px-4 py-3 text-right">
                      {!revoked && (
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="bg-background border-border text-foreground max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">Generate a new key for this app.</DialogDescription>
          </DialogHeader>
          <div className="py-2">
            ...
            <div className="flex gap-2 mt-1">
              {(["dev", "prod"] as const).map((env) => (
                <button key={env} type="button" onClick={() => setKeyEnv(env)}
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${keyEnv === env ? "bg-accent/20 text-accent border border-accent/40" : "bg-white/5 text-muted-foreground border border-white/10 hover:bg-white/10"}`}>
      ...
      <Dialog open={copyOpen} onOpenChange={(open) => { if (!open) { setNewKeyPlain(""); setCopyOpen(false); } }}>
        <DialogContent className="bg-background border-border text-foreground max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">This is the only time you'll see this key.</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={!!revokeTarget} onOpenChange={(open) => { if (!open) setRevokeTarget(null); }}>
        <DialogContent className="bg-background border-border text-foreground max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">This cannot be undone. Integrations using this key will break.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
    ...
    <div className="rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.10)" }}>
      {!logs?.length ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground text-sm">No runtime logs for this app yet.</p>
        </div>
      ) : (
        <table className="w-full text-sm">
          <thead><tr className="border-b border-white/10">
          ...
          <tbody>
            {logs.map((log: any) => (
              <tr key={log.id} className="border-b border-white/5 last:border-0">
                <td className="px-4 py-3">
                  <Badge variant="outline" className="text-xs border-white/20 text-muted-foreground">{log.action}</Badge>
                </td>
...
function PermissionsTab({ appId }: { appId: string }) {
  ...
  const statusBadge = (status: string) => {
    ...
    return <Badge className={`text-xs ${colors[status] || "bg-white/10 text-muted-foreground"}`}>{status}</Badge>;
  };
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-2">
            {scopes.map((s) => (
              <div key={s.scope_key} className="flex items-center justify-between px-3 py-2 rounded-lg bg-white/[0.02]">
                <code className="text-xs text-muted-foreground font-mono">{s.scope_key}</code>
                {statusBadge(s.status)}
              </div>
            ))}
          </div>
        ) : (
          <p className="text-muted-foreground text-xs">No scopes configured.</p>
        )}
      </div>
      ...
      <div className="rounded-xl p-5 bg-white/[0.02] border border-white/10">
        ...
        {providers && providers.length> 0 ? (
          <div className="space-y-2">
            {providers.map((p) => (
              <div key={p.id} className="flex items-center justify-between px-3 py-2 rounded-lg bg-white/[0.02]">
                <code className="text-xs text-muted-foreground font-mono">{p.provider_key}</code>
          ...
          <p className="text-muted-foreground text-xs">No provider permissions configured.</p>
        )}
      </div>
    </div>
  );
