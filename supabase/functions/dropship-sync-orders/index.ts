...
Deno.serve(async (req) => {
  ...
  try {
    const { data: jobs, error: batchErr } = await supabase.rpc("process_dropship_order_sync_batch", { p_limit: 25 });

    if (batchErr) {
      console.error("process_dropship_order_sync_batch error:", batchErr);
    ...
    for (const job of jobList) {
      ...
      if (!adapter) {
        await supabase.rpc("complete_dropship_order_sync_job", {
          p_job_id: job.id,
          p_success: false,
      ...
      try {
        const result = await adapter.getOrderStatus(job.provider_order_id);

        await supabase.rpc("complete_dropship_order_sync_job", {
          p_job_id: job.id,
          p_success: true,
          p_new_status: result.status,
          p_tracking: result.tracking,
          p_provider_payload: result.raw,
        });
        successCount++;
      } catch (err: unknown) {
    ...
    console.error("sync-orders fatal:", msg);
