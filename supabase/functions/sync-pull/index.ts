...
Deno.serve(async (req) => {
  ...
  if (Number.isNaN(sinceDate.getTime())) return json({ error: "invalid_since" }, 400, req);
  ...
  if (error) return json({ error: "pull_failed", detail: error.message }, 500, req);

  const cursor = data && data.length > 0
    ? data[data.length - 1].updated_at
    : sinceDate.toISOString();

  return json({ catalogs: data ?? [], cursor }, 200, req);
});