CREATE OR REPLACE FUNCTION public.get_published_surface(
  p_publish_id uuid DEFAULT NULL::uuid,
  p_surface_id uuid DEFAULT NULL::uuid
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
...
DECLARE
  ...
  dom record;
BEGIN
  -- Find the active publish record
  ...
  -- Fetch linked surface and domain
  SELECT * INTO surf
