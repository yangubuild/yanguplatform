...
BEGIN
  ...
  -- 5. Primary page must have at least one visible section
  ...
  -- 6. Check for orphan sections
  SELECT count(*) INTO v_orphan_count
  FROM builder_sections bsec
  WHERE NOT EXISTS (
    SELECT 1 FROM builder_pages bp WHERE bp.id = bsec.page_id AND bp.surface_id = p_surface_id
  )
  AND bsec.page_id IN (
    SELECT bp2.id FROM builder_pages bp2 WHERE bp2.surface_id = p_surface_id
  );

  -- ═══ SLUG ═══
  v_normalized_slug := coalesce(nullif(trim(p_slug), ''), v_surface.slug);
  ...
  SELECT jsonb_build_object(
    'surface', jsonb_build_object(
      ...
      'theme', bs.theme,
      'seo_title', bs.seo_title,
      'seo_description', bs.seo_description,
        ...
        SELECT
          ...
          jsonb_build_object(
            ...
            'sections', coalesce((
              SELECT jsonb_agg(
                jsonb_build_object(
                  'section_type', deduped.section_type,
                  'schema', deduped.schema,
                  'position', deduped.position
                ) ORDER BY deduped.position
              )
              FROM (
                -- Deduplicate: for each section_type, keep only the row with
                -- the latest updated_at (or lowest position as tiebreaker).
                -- This prevents duplicate main_content/gallery/etc rows.
                SELECT DISTINCT ON (bsec.section_type)
                  bsec.section_type,
                  bsec.schema,
                  bsec.position
                FROM builder_sections bsec
                WHERE bsec.page_id = bp.id AND bsec.is_visible = true
                ORDER BY bsec.section_type, bsec.updated_at DESC, bsec.position ASC
              ) deduped
            ), '[]'::jsonb)
          ) AS page_data
