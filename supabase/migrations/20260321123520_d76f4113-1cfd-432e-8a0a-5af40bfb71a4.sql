...
-- Drop old search_entities to change return type
...
-- Re-sync all surfaces to pick up new primary_category mapping
DO $$
DECLARE
  r record;
BEGIN
  FOR r IN
    SELECT s.id FROM public.surfaces s WHERE s.draft_slug IS NOT NULL
  LOOP
