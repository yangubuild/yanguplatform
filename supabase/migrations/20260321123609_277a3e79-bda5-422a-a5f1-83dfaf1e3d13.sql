...
-- Fix sync_searchable_entity: derive primary_category from builder_surface_type, pull seo_description as fallback
...
BEGIN
  ...
  v_bst := COALESCE(v_builder.surface_type::text, v_surface.surface_type);
  ...
  IF v_builder.id IS NOT NULL THEN
    ...
    v_description := NULLIF(BTRIM(v_builder.description), '');
    IF v_description IS NULL THEN
      v_description := NULLIF(BTRIM(v_builder.seo_description), '');
    END IF;
  ...
  -- primary_category: industry or mapped from builder_surface_type
  v_category := v_industry;
  IF v_category IS NULL THEN
    v_category := CASE v_bst
      WHEN 'eshop' THEN 'shop'
      WHEN 'emenu' THEN 'shop'
      WHEN 'esite' THEN 'services'
      WHEN 'estore' THEN 'store'
      WHEN 'store_listing' THEN 'store'
      WHEN 'live_bio' THEN 'influencer'
      WHEN 'live_selling' THEN 'creator'
      WHEN 'community_group' THEN 'community'
      WHEN 'community_listing' THEN 'community'
      WHEN 'studio_showcase' THEN 'studio'
      ELSE NULL
    END;
  END IF;

  v_entity_type := public.derive_entity_type(v_bst);
  v_entity_subtype := public.derive_entity_subtype(v_bst, v_industry);
...
DECLARE r record;
BEGIN
  FOR r IN SELECT s.id FROM public.surfaces s WHERE s.draft_slug IS NOT NULL
  LOOP PERFORM public.sync_searchable_entity(r.id); END LOOP;
END $$;
